angular.module('App').controller('RootCtrl', function ($rootScope, $scope, $mdSidenav, $mdToast, $mdDialog, $cookies, request, focus) {
    ...
    self.sidenav = {
        actions: [
            ...
            {name: 'SHIPPING', icon: 'local_shipping', link: '#shipping', sub: false},
    ...
    root.sort_by = [
        ...
        {id:2, label : "Price : Low to High", column:"price", order:"ASC"},
        {id:3, label : "Price : High to Low", column:"price", order:"DESC"},
        {id:4, label : "Discount", column:"price_discount", order:"DESC"}
    ];
    ...
    // for editing news info
    root.setCurSettingTab = function (index) {
        $cookies.put(root.base_url + 'cur_setting_tab', index);
    };
    root.getCurSettingTab = function () {
        var index = $cookies.get(root.base_url + 'cur_setting_tab');
    ...
    root.showProgressDialog = function (){
        $mdDialog.show({
            controller : function ($scope, $mdDialog){ },
            parent: angular.element(document.body), clickOutsideToClose: false,
            template:
            '<md-dialog ng-cloak aria-label="loading">' +
