...
class CLIENT extends REST{
    ...
    public function __construct($db) {
       ...
       // $this->ricecategory = new RiceCategory($this->db);
    ...
    public function findAllProduct(){
        if($this->get_request_method() != "GET") $this->response('',406);
        $limit = isset($this->_request['count']) ? ((int)$this->_request['count']) : 100;
        $page = isset($this->_request['page']) ? ((int)$this->_request['page']) : 1;
        $q = isset($this->_request['q']) && $this->_request['q'] != null ? ($this->_request['q']) : "";
    ...
    public function findAllCategory(){
      ...
      /* Response Rice Category */
    public function findRiceCategory(){
        if($this->get_request_method() != "GET") $this->response('',406);
        $categories = $this->category->findAllrice();
        $response = array(
            'status' => 'success', 'categories' => $categories
        );
        $this->show_response($response);
    }
     /* Response Vegetable Category */
    public function findAllVegitable(){
        if($this->get_request_method() != "GET") $this->response('',406);
        $categories = $this->category->findAllVegitable();
        $response = array(
            'status' => 'success', 'categories' => $categories
        );
        $this->show_response($response);
    }
     /* Response SPN Special Category */
    public function findspncategory(){
        if($this->get_request_method() != "GET") $this->response('',406);
        $categories = $this->category->findAllspncategory();
        $response = array(
            'status' => 'success', 'categories' => $categories
        );
        $this->show_response($response);
    }
     /* Response Plastic Items Category */
    public function findPlcategory(){
        if($this->get_request_method() != "GET") $this->response('',406);
        $categories = $this->category->findAllplasticcategory();
        $response = array(
            'status' => 'success', 'categories' => $categories
        );
        $this->show_response($response);
    }
  /* Response Fish n Meat Category */
    public function findFnMcategory(){
        if($this->get_request_method() != "GET") $this->response('',406);
        $categories = $this->category->findAllFnMcategory();
        $response = array(
            'status' => 'success', 'categories' => $categories
        );
        $this->show_response($response);
    ...
    public function findrice(){
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->product->allCountPlainForClient($q, $category_id);
        $products = $this->product->findAllRices($limit, $offset, $q, $category_id, $column, $order);
 ...
 public function findVegitables(){
         if($this->get_request_method() != "GET") $this->response('',406);
        $limit = isset($this->_request['count']) ? ((int)$this->_request['count']) : 100;
        $page = isset($this->_request['page']) ? ((int)$this->_request['page']) : 1;
        $q = isset($this->_request['q']) && $this->_request['q'] != null ? ($this->_request['q']) : "";
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->product->allCountPlainForClient($q, $category_id);
        $products = $this->product->findAllVegetables($limit, $offset, $q, $category_id, $column, $order);
     ...
     public function findFnM(){
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->product->allCountPlainForClient($q, $category_id);
        $products = $this->product->findAllFnM($limit, $offset, $q, $category_id, $column, $order);
    ...
    public function findworders(){
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->product->allCountPlainForClient($q, $category_id);
        $products = $this->product->findAllWO($limit, $offset, $q, $category_id, $column, $order);
     ...
     public function findOthers(){
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->product->allCountPlainForClient($q, $category_id);
        $products = $this->product->findOthers($limit, $offset, $q, $category_id, $column, $order);
     ...
     public function findSPNItems(){
        ...
        $offset = ($page * $limit) - $limit;
        $count_total = $this->product->allCountPlainForClient($q, $category_id);
        $products = $this->product->findSpnItems($limit, $offset, $q, $category_id, $column, $order);
